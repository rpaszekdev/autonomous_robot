...
```bash
# Mock camera (no webcam)
python -m robot.main --simulate

# Real webcam
python -m robot.main --simulate --webcam

# Specific webcam index (if you have multiple)
...
- `--webcam` — use the real webcam via OpenCV (Mac/Linux)
- `--webcam-index N` — pick a specific webcam (default 0)
- `--list-cameras` — scan indices 0–3 and exit
